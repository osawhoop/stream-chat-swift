...
open class _ComposerVC<ExtraData: ExtraDataTypes>: _ViewController,
    ...
    @objc open func showAttachmentsPicker(sender: UIButton) {
        ...
        if content.attachments.contains(where: { $0.type == .file }) {
            showFilePicker()
        } else if content.attachments.contains(where: { $0.type == .image || $0.type == .media }) {
            showMediaPicker()
        } else {
    ...
    open func imagePickerController(
        ...
        do {
            if let imageURL = info[.imageURL] as? URL {
                ...
                let attachment = try AnyAttachmentPayload(localFileURL: mediaURL, attachmentType: .media)
                content.attachments.append(attachment)
            }
        } catch {
